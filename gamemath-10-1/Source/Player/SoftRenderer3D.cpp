...
void SoftRenderer::LoadScene3D()
{
	...
	mainCam.GetTransform().SetPosition(Vector3(0.f,0.f,500.f));
	mainCam.GetTransform().SetRotation(Rotator(180.f, 0.f, 0.f));
...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	//입력에 따른 트랜스폼 변경
	goPlayer.GetTransform().AddPosition(Vector3::UnitZ * input.GetAxis(InputAxis::ZAxis) *
		moveSpeed * InDeltaSeconds);
	goPlayer.GetTransform().AddPitchRotation(input.GetAxis(InputAxis::WAxis) *
		rotateSpeed * InDeltaSeconds);

	camera.GetTransform().AddYawRotation(-input.GetAxis(InputAxis::XAxis) *
		rotateSpeed * InDeltaSeconds);
	camera.GetTransform().AddPitchRotation(-input.GetAxis(InputAxis::YAxis) *
		rotateSpeed * InDeltaSeconds);
