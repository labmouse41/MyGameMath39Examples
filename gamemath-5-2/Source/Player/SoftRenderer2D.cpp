...
void SoftRenderer::Render2D()
{
	...
	Vector2 sBasics1(currentScale, 0.f);
	Vector2 sBasics2(0.f, currentScale);
	Matrix2x2 sMatrix(sBasics1, sBasics2);

	float sin, cos;
	Math::GetSinCos(sin, cos, currentDegree);
	Vector2 rBasics1(cos, sin);
	Vector2 rBasics2(-sin, cos);
	Matrix2x2 rMatrix(rBasics1, rBasics2);

	Vector2 shBasics1 = Vector2::UnitX;
	Vector2 shBasics2(currentShear, 1.f);
	Matrix2x2 shMatrix(shBasics1, shBasics2);

	Matrix2x2 cMatrix = shMatrix * rMatrix * sMatrix;

	float invScale = 1.f / currentScale;
	Vector2 isBasics1(invScale, 0.f);
	Vector2 isBasics2(0.f, invScale);
	Matrix2x2 isMatrix(isBasics1, isBasics2);

	Matrix2x2 irMatrix = rMatrix.Transpose();

	Vector2 ishBasics1 = Vector2::UnitX;
	Vector2 ishBasics2(-currentShear, 1.f);
	Matrix2x2 ishMatrix(ishBasics1, ishBasics2);
